...
import {additiveFromTotalPercent,hexToRgb,mixHex,mixMany,scaleRecipe} from './color.ts';
describe('physical batch math',()=>{
  ...
  it('preserves total proportions for multi-additive mixes',()=>assert.equal(mixMany('#3D695F',128,[{hex:'#FFFFFF',oz:8},{hex:'#888888',oz:8}]),mixMany('#3D695F',64,[{hex:'#FFFFFF',oz:4},{hex:'#888888',oz:4}])));
  it('scales the top three 8 oz-base recipes exactly',()=>assert.deepEqual({gypsum:scaleRecipe(128,8,8),ivory:scaleRecipe(128,16,8),combo:[scaleRecipe(128,12,8),scaleRecipe(128,4,8)]},{gypsum:.5,ivory:1,combo:[.75,.25]}));
});
